...
void VRToolButtonWidget::Render(Urbaxio::Renderer& renderer, Urbaxio::TextRenderer& textRenderer, const glm::mat4& panelTransform, const glm::mat4& view, const glm::mat4& projection, float alpha) {
    ...
    // --- 1. Render Sphere ---
    float sphereDiameter = size_.y * 0.8f; // Sphere is slightly smaller than button height
    glm::vec3 sphereLocalPos = localPosition_ - glm::vec3(size_.x * 0.5f - sphereDiameter * 0.6f, 0, 0);
    ...
    // --- 2. Render Text (only if hovering) ---
    if (textHoverAlpha_ > 0.01f) {
        glm::vec3 textWorldPos = panelTransform * glm::vec4(localPosition_, 1.0f);
        float textHeight = size_.y * panelLocalScale * 0.6f; // Reduced size by 40%
        glm::vec4 textColor = glm::vec4(1.0f, 1.0f, 1.0f, alpha * textHoverAlpha_);
        textRenderer.AddText(text_, textWorldPos, textColor, textHeight, view);
    }
}
